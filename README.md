...
为了使每一列的迭代压缩满足上述的约束条件，算法使用三个队列分别保存当前迭代中当前列可参与下一列运算的半加器的cout、3-2/4-2压缩器的cout、不能参与下一列运算的cout及4-2压缩器的co，前两个队列按照约束参与本列的迭代压缩，本列的迭代压缩结束时将所有队列中剩余的进位补进bit阵列中。本算法着重于性能，但并不是最优性能，在尝试不同组合策略中可明显观察到性能面积互换规律，且已发现了一种策略的性能略优，面积略大（遗失）。
# 在Vivado平台下与乘法器IP对比
